...
func MS2Search(queryMS2 string, threshold string, tolerance string, prefilterThreshold string) (string, error) {
	// 检查Python进程是否已初始化
	if p == nil {
		errMsg := "RDkit进程未初始化"
		utils.Log(errMsg)
	...
	if err != nil {
		utils.LogError(err)
		return "", fmt.Errorf("MS2相似度搜索失败: %v", err)
	}
...
func MS2SearchByFingerprint(fingerprintJson string, threshold string, tolerance string, prefilterThreshold string) (string, error) {
	// 检查Python进程是否已初始化
	if p == nil {
		errMsg := "RDkit进程未初始化"
		utils.Log(errMsg)
	...
	// 发送MS2指纹搜索请求
	requestData := map[string]interface{}{
		"action":              "ms2_search_by_fingerprint",
	...
	if err != nil {
		utils.LogError(err)
		return "", fmt.Errorf("MS2指纹搜索失败: %v", err)
	}
